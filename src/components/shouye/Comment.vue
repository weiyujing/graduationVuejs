<template>
    <div class="comment-box">
        <div v-if="isSignIn===0" class="signInText" @click="toLogin">登录留言吧</div>
...
import Login from './Login'
  ...
  toClose(){
      ...
      this.getUserInfo();
            ...
            publicComment() {
                this.$axios.post('/api/comment/public',{
                    stock_id:this.$route.params.stock,
            ...
            getCommentList() {
                this.$axios.post('/api/comment/list',{
                        stock_id:this.$route.params.stock       
                }).then((res)=>{
                    console.log(res)
                    if(res.data.code === 0){
   ...
   callbackName: "refreshdatastr",
         timeout: 50000,
  output:"jsonp"
}
       this.$jsonp('http://webstock.quote.hermes.hexun.com/a/minute?code=sse600000&start=20181026000000&number=500&callback=refreshdatastr').then(
        (json) => {
          console.log(json)
        }).catch(err => {
          console.log(err)
